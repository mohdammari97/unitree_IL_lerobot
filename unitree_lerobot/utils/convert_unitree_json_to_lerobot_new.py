...
class JsonDataset:
    ...
    def _init_paths(self) -> None:
        """Identify episode directories that contain data.json."""

        self.episode_paths = []

        # 遍历 raw_dir 下面的一级子目录
        for episode_dir in glob.glob(os.path.join(self.data_dirs, "*")):
            if not os.path.isdir(episode_dir):
                continue

            json_path = os.path.join(episode_dir, self.json_file)

            # 只把包含 data.json 的文件夹认作 episode
            if os.path.isfile(json_path):
                self.episode_paths.append(episode_dir)
        ...
        for sample_data in episode_data["data"]:
            ...
            for part in parts :
                if part in sample_data[key] and sample_data[key][part] is not None:
                    for attr in ["qpos", "qvel", "torque"]:
                        values = sample_data[key][part].get(attr)
                        if values:
                            arr = np.array(values, dtype=np.float32)
                            data_array = np.concatenate([data_array, arr])
